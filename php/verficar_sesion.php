...
while($not = mysqli_fetch_assoc($notifications)){
  if(($not["tipo"] == DRIVER) || ($not["tipo"] == PASSENGER)){
    ...
    if($transaction["tipo"] == 1){
      ...
      $text = "Se te depositó el viaje de ".$trip["origen"]." a ".$trip["destino"].".<br>Podes ver mas información en tu <a href=\'/vistas/ver_resumen_cuenta.php\'>resumen de cuenta</a>";
    }
  } else if($not["tipo"]==CALIFICATION_PASSENGER){
    $text = "Calificá al conductor de tu último viaje para poder seguir viajando y ahorrando!";
  } else if($not["tipo"]==CALIFICATION_DRIVER){
    $text = "Calificá a los pasajeros de tu último viaje para poder seguir viajando y ahorrando!";
  }
